...
## Status rozwoju (aktualizacja 0.1.0)
...
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | --------------- | ------ |
| Fundament wtyczki                                               | Struktura plików, rejestracja wtyczki, autoloader, podstawowe hooki, logowanie debug.           | 0.1.0           | 10%   |
| Panel administratora – menu główne                              | Dodanie menu "Estate Office CRM" oraz ekranów placeholder.                                     | 0.1.0           | 0%    |
| Baza danych – tabele customowe                                  | Projekt i wdrożenie tabel dla nieruchomości, klientów, umów, poszukiwań, agentów.               | 0.2.0           | 0%    |
...
## Plan iteracyjny
- **Wersja 0.1.0 (bieżąca)**: Budowa fundamentów – plik główny wtyczki, ładowanie klas, konfiguracja debug oraz przygotowanie miejsc pod menu administratora. Testy funkcjonalne rozpoczną się po implementacji pierwszych ekranów (szacowany termin: wersja 0.2.0).
- **Wersja 0.2.0**: Utworzenie dedykowanych tabel w bazie danych i modelu danych. Planowane wewnętrzne testy instalatora oraz aktualizacji schematu.
...
- **Wprowadzone w tej wersji (0.1.0)**: utworzenie roadmapy i fundamentów strukturalnych wtyczki (plik główny, klasy ładowania, rejestracja menu placeholder).
- **Następne kroki**: prace nad strukturą bazy danych i modułem instalatora (wersja 0.2.0).
- **Harmonogram testów**: pierwsze kompleksowe testy funkcjonalne planowane wraz z zakończeniem wersji 0.3.0; wcześniejsze wersje obejmują testy jednostkowe komponentów backendowych.
