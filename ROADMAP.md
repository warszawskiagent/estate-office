...
## Status rozwoju (aktualizacja 1.0.5)
...
| Publiczny katalog agentów | Shortcode, strona publiczna i profil agenta | 1.0.4 | 100% |
...
- **Wersja 1.0.5 (bieżąca)**: Uzupełnienie formularza nieruchomości o brakujące pola ze specyfikacji (typ domu, pełny stan prawny, automatyczna cena za m²), interaktywna mapa Google oraz podgląd galerii; dostosowanie schematu bazy i CSS formularzy.
- **Wersja 1.0.4 (zrealizowana)**: Publiczny katalog agentów z shortcode i dedykowaną stroną, rozbudowane metadane kontaktowe agentów oraz odświeżony formularz w panelu administratora.
- **Wersja 1.0.3 (zrealizowana)**: Przywrócenie pełnych list w portalu CRM (nieruchomości, umowy, klienci, poszukiwania) z filtrami, paginacją i widokami szczegółowymi, rozbudowa frontowych formularzy (w tym etapów kreatora) oraz nowa warstwa CSS dla portalu.
...
- **Wprowadzone w tej wersji (1.0.5)**: komplet parametrów w formularzu nieruchomości (typ domu, wariant "Inne" ze szczegółem, automatyczna cena za m²), interaktywna mapa Google w oparciu o klucz API z ustawień, dynamiczny podgląd galerii oraz odświeżony CSS sekcji formularza.
- **Wprowadzone wcześniej (1.0.4)**: publiczny katalog agentów ze stroną główną i widokiem profilu, shortcode `[estate_office_agents]`, rozwinięty formularz agenta (teleadres, strona WWW, lokalizacja) oraz spójny styling w panelu i na froncie.
- **Wprowadzone wcześniej (1.0.3)**: przywrócone i odświeżone listy CRM w portalu (filtry, wyszukiwarka, paginacja, widoki szczegółowe), rozszerzone formularze frontowe nieruchomości/poszukiwań wraz z obsługą kreatora umów oraz nowy stylesheet portalu zapewniający spójny wygląd i responsywność.
