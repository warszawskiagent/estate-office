...
use EstateOffice\Controllers\Property_Controller;
use WP_Error;
use WP_REST_Request;
use WP_REST_Server;
...
/**
 * Exposes property CRUD endpoints.
 */
class Properties_Controller extends Base_Controller {
    /**
     * Property controller instance.
     *
     * @var Property_Controller
     */
    private Property_Controller $controller;

    public function __construct() {
        $this->controller = new Property_Controller();
    }
    ...
    public function get_item( WP_REST_Request $request ) {
        ...
        if ( empty( $item ) ) {
            return new WP_Error( 'estate_office_not_found', __( 'Nieruchomość nie została znaleziona.', 'estate-office' ), [ 'status' => 404 ] );
        }
    ...
    public function create_item( WP_REST_Request $request ) {
        ...
        if ( ! $id ) {
            return new WP_Error( 'estate_office_create_failed', __( 'Nie udało się utworzyć nieruchomości.', 'estate-office' ), [ 'status' => 500 ] );
        }
    ...
    public function update_item( WP_REST_Request $request ) {
        ...
        if ( ! $this->controller->update( $id, $data ) ) {
            return new WP_Error( 'estate_office_update_failed', __( 'Nie udało się zaktualizować nieruchomości.', 'estate-office' ), [ 'status' => 500 ] );
        }
    ...
    public function delete_item( WP_REST_Request $request ) {
        ...
        if ( ! $this->controller->delete( $id ) ) {
            return new WP_Error( 'estate_office_delete_failed', __( 'Nie udało się usunąć nieruchomości.', 'estate-office' ), [ 'status' => 500 ] );
        }
