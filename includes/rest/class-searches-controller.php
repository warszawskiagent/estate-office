...
use EstateOffice\Controllers\Search_Controller;
use WP_Error;
use WP_REST_Request;
use WP_REST_Server;
...
/**
 * Exposes saved search CRUD endpoints.
 */
class Searches_Controller extends Base_Controller {
    ...
    public function get_item( WP_REST_Request $request ) {
        ...
        if ( empty( $item ) ) {
            return new WP_Error( 'estate_office_not_found', __( 'Zapisane wyszukiwanie nie zostało znalezione.', 'estate-office' ), [ 'status' => 404 ] );
        }
    ...
    public function create_item( WP_REST_Request $request ) {
        ...
        if ( ! $id ) {
            return new WP_Error( 'estate_office_create_failed', __( 'Nie udało się utworzyć wyszukiwania.', 'estate-office' ), [ 'status' => 500 ] );
        }
    ...
    public function update_item( WP_REST_Request $request ) {
        ...
        if ( ! $this->controller->update( $id, $data ) ) {
            return new WP_Error( 'estate_office_update_failed', __( 'Nie udało się zaktualizować wyszukiwania.', 'estate-office' ), [ 'status' => 500 ] );
        }
    ...
    public function delete_item( WP_REST_Request $request ) {
        ...
        if ( ! $this->controller->delete( $id ) ) {
            return new WP_Error( 'estate_office_delete_failed', __( 'Nie udało się usunąć wyszukiwania.', 'estate-office' ), [ 'status' => 500 ] );
        }
