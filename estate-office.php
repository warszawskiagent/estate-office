...
final class EstateOffice_CRM_Plugin {
    ...
    private function get_crm_styles() {
        ...
        .estateoffice-crm__actions {
            margin-bottom: 14px;
        }
        .estateoffice-crm__actions a {
            text-decoration: none;
            background: #0f766e;
            color: #fff;
            padding: 10px 16px;
        ...
        .estateoffice-crm__search-form label {
            grid-column: 1 / -1;
        }
    ...
    public function render_crm_shortcode( $atts ) {
        ...
        $add_contract_link = $links['contracts'];
        if ( '#' !== $add_contract_link ) {
            $add_contract_link = add_query_arg( 'new', '1', $add_contract_link );
        }
        ...
        <div class="estateoffice-crm">
            ...
            <section class="estateoffice-crm__content">
                <div class="estateoffice-crm__actions">
                    <a href="<?php echo esc_url( $add_contract_link ); ?>">Dodaj nową Umowę</a>
                </div>
    ...
    private function get_search_term() {
        return isset( $_GET['eo_q'] ) ? sanitize_text_field( wp_unslash( $_GET['eo_q'] ) ) : '';
    }

    private function render_list_search_form( $placeholder ) {
        $current_value = $this->get_search_term();
        ?>
        <form method="get" class="estateoffice-crm__search-form">
            <?php foreach ( $_GET as $key => $value ) : ?>
                <?php if ( 'eo_q' === $key ) : ?>
                    <?php continue; ?>
                <?php endif; ?>
                <input type="hidden" name="<?php echo esc_attr( sanitize_key( $key ) ); ?>" value="<?php echo esc_attr( is_scalar( $value ) ? wp_unslash( $value ) : '' ); ?>">
            <?php endforeach; ?>
            <label for="eo_q">Wyszukiwanie</label>
            <input id="eo_q" type="search" name="eo_q" value="<?php echo esc_attr( $current_value ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>">
    ...
    private function render_dashboard_view() {
        ...
        $top_agents = $wpdb->get_results(
            "SELECT agent_id, COUNT(*) as offers_total FROM {$wpdb->prefix}eo_properties WHERE agent_id IS NOT NULL GROUP BY agent_id ORDER BY offers_total DESC LIMIT 5"
        );
        ?>
        <div class="estateoffice-crm__section">
            <h3>Podsumowanie CRM</h3>
            <table>
                <tbody>
                    <tr><th>Liczba nieruchomości</th><td><?php echo esc_html( $properties_count ); ?></td></tr>
                    <tr><th>Aktywne umowy</th><td><?php echo esc_html( $contracts_count ); ?></td></tr>
                    <tr><th>Poszukiwania</th><td><?php echo esc_html( $searches_count ); ?></td></tr>
                    <tr><th>Klienci</th><td><?php echo esc_html( $clients_count ); ?></td></tr>
                </tbody>
            </table>
        </div>

        <div class="estateoffice-crm__section">
            <h3>Najlepsi agenci</h3>
            <?php if ( empty( $top_agents ) ) : ?>
                <p>Brak danych agentów.</p>
            <?php else : ?>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php foreach ( $top_agents as $agent ) : ?>
                            <tr>
                                <td><?php echo esc_html( $this->get_agent_display_name( $agent->agent_id ) ); ?></td>
                                <td><?php echo esc_html( $agent->offers_total ); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    private function render_clients_view() {
        ...
        if ( '' !== $search_term ) {
            ...
            $clients = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}eo_clients
                    WHERE first_name LIKE %s
                    OR last_name LIKE %s
                    OR company_name LIKE %s
                    OR phone LIKE %s
                    OR email LIKE %s
                    OR address_city LIKE %s
        ...
        <div class="estateoffice-crm__section">
            ...
            <?php $this->render_list_search_form( 'Szukaj po imieniu, nazwisku, telefonie, e-mailu, mieście...' ); ?>
    ...
    private function render_contracts_view() {
        ...
        if ( '' !== $search_term ) {
            ...
            $contracts = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}eo_contracts
                    WHERE number LIKE %s
        ...
        <div class="estateoffice-crm__section">
            <h3>Lista umów</h3>
            <?php $this->render_list_search_form( 'Szukaj po numerze umowy, typie transakcji, dacie...' ); ?>
    ...
    private function render_properties_view() {
        ...
        if ( '' !== $search_term ) {
            ...
            $properties = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}eo_properties
                    WHERE address_street LIKE %s
                    OR address_number LIKE %s
                    OR address_city LIKE %s
                    OR CAST(price AS CHAR) LIKE %s
                    OR CAST(area AS CHAR) LIKE %s
                    OR CAST(rooms AS CHAR) LIKE %s
                    ORDER BY created_at DESC LIMIT 50",
                    $wildcard,
                    $wildcard,
                    $wildcard,
        ...
        <div class="estateoffice-crm__section">
            ...
            <?php $this->render_list_search_form( 'Szukaj po adresie, cenie, metrażu, pokojach...' ); ?>
    ...
    private function render_searches_view() {
        ...
        if ( '' !== $search_term ) {
            ...
            $searches = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}eo_searches
                    WHERE property_type LIKE %s
                    OR location LIKE %s
                    OR transaction_type LIKE %s
                    OR CAST(budget_min AS CHAR) LIKE %s
                    OR CAST(budget_max AS CHAR) LIKE %s
                    ORDER BY created_at DESC LIMIT 50",
                    $wildcard,
                    $wildcard,
                    $wildcard,
        ...
        <div class="estateoffice-crm__section">
            ...
            <?php $this->render_list_search_form( 'Szukaj po rodzaju, budżecie, lokalizacji, typie transakcji...' ); ?>
